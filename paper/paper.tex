...
\address[1]{\orgdiv{Energy Geosciences Division}, \orgname{Lawrence Berkeley National Laboratory}, \orgaddress{\state{CA}}, \country{USA}}
...
\abstract[Abstract]{ Multiphase simulation is hard.
...
Note that the definition of enthalpy as a per-mass quantity means the mixture needs to be weighted by the
...
\end{quote}
{\bf Remark:} This interpretation breaks down with the existence of super-saturated phases in non-stable regions: e.g., it is possible to
...
2006 formulation for the Ice Ih phase to build the dataset.\footnote{The
implementation is publicly available as a stand-alone library at \url{https://github.com/afqueiruga/equations\_of\_state}.} (Their
...
expressions. The family of cubic equations of state for non-ideal
gases, such as the Peng-Robinson equation \cite{peng_new_1976}, are
expressed as functions for $p$ of $V$ (a proxy for $\rho$) and
$T$. Since $\rho$ is rarely a primary variable, these expressions must be inverted for
...
abound--the mathematical theory is not straight forward. Passing
...
\end{tabular}
\end{center}
Solving the balance laws with this caveat requires the incorporation
...
\end{figure}
The discretized balance equations become a nonlinear function of
continuous variables $X$ and discrete indices $phase$:
\begin{equation}
R(X ; phase) = 0
\end{equation}
where the meaning of $X$ is dictated by $phase$. Solving this equation
requires first iterating on the differentiable part by solving the
Jacobian matrix,
\begin{equation}
  \frac{\partial R}{\partial X}\Delta X^{k+1/2} = -R(X^k ; phase^k)
\end{equation}
and the iterating the non-differentiable state machine part:
...
Algorithm \ref{alg:statemachine}. This converges quadratically in
situations where no phases changes occur, but execution of
Eq. \ref{eq:fsm} breakdown into linear convergence or even divergence.
This often contains the presence of ``tricks'' such as overshoots to
push the grid blocks over the edge, $P_{new}=(1+10^{-6} )P_{old}$, or
artificially smoothing the phase boundaries
\citep{moridis_simulation_2019}. 
 Besides the computational difficulties, from a software
engineering standpoint, the long and winding logical branches are very
difficult to maintain and extend with new physics. The presence of the state machine and implicit equations inside of the
material descriptions also requires numerical differentiation, which is
a further numerical challenge. The engineering
difficulty further makes it difficult to improve the efficiencies of the implementations.
...
has three unknowns with two second-order equations and a constraint,
\begin{align}
...
\begin{figure}
  \center
\includegraphics[width=5in]{../figures/autoencoder_balance_detailed.png}
...
Re-parameterization}\label{header-n3294}}
...
\(q_1\) and \(q_2\) to require no auxiliary phase index to define the system
...
step is mini-batched stochastic gradient descent on the entire deep network, assembling the losses on minibatches of the data, $X_{mini}$,
\begin{equation}
...
unfolded into the new parameterization in Figure
\ref{fig:foursurfaces}. These surfaces were designed to be smooth
enough to solve effectively.
With the trained model, we solve for \(q_1(t)\) and \(q_2(t)\) such that:
\begin{align}
\partial_t \rho(q_1,q_2) & = \mathbf{k} \left( p_\infty-p(q_1,q_2) \right)  + r \label{eq:massbalanceq}\\
\partial_t \left(\rho h(q_1,q_2)-p\right) &
                                            =\mathbf{k}_T(T_\infty-T(q_1,q_2))+ s + \frac{\rho h}{h}\left(\mathbf{k}_p(p_\infty - p) + r\right) \label{eq:heatbalanceq}
\end{align}
...
$eos(T,p,\rho,h)=0$ is the material constraint. The decoder part of the network is inserted into the balance law
\begin{equation}
\partial_t (D(q)) = r(D(q))
\end{equation}
...
to its simplicity and L-stability. We make the same choice because the topic of
...
value \(q^0\) at \(t\),
\begin{equation}
0 = R(q^i) = m(q^{i}) - m(q^{0}) - \Delta t \, r(q^i) \label{eq:bweuler}
\end{equation}
...
\begin{equation}
\frac{\partial R}{\partial q} \Delta q^{k+1} = - R(q^k) \label{eq:jacobianq}
\end{equation}
with the update
\begin{equation}
...
models; implementing equations \ref{eq:massbalanceq}, \ref{eq:heatbalanceq}, and \ref{eq:bweuler } requires only a few lines
of human-written code. The iteration for \ref{eq:jacobianq} can be constructed entirely with automatic differentiation because the model is fully differentiable.
...
non-prototype simulator.
Then, the simulator must determine $q(t=0)$ from these two
values, but the encoder only works when all four values are on the
...
in the EOS space is in a solid, the iteration will diverge.
To address this issue, the solver performs multiple starts with an initial condition in each
phase and equilibrium, discard points that diverge, and then picks the
...
values that represent middle points for each phase, \footnote{The multistart method could be specified without any prior knowledge
with random points, but picking start points with prior knowledge was necessary to test the cause of the divergence.}
\begin{equation}
...
\end{array}\right)... \right\}
  \end{equation}
Each of the possible guess points is used to fill in the two
...
\begin{figure}
  \centering
  \includegraphics[width=4in]{../figures/workflow_vector.pdf}
...
The ``best'' model can then be ``shipped''  to be automatically selected by the final end user,
...
Approaching this from a building-block problem, we can craft models
with baked in physical knowledge.
The entire system has another level of complexity
...
We foresee this end-to-end approach will allow the system to develop
...
session.run and a very complicated graph structure for the
polynomials. To implement a fully-fledged reservoir simulator, new programming libraries are being explored, such as
Julia with Flux and Zygote for machine learning, or the JAX library in Python.
