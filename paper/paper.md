...
$p,T$ is the usual prefered choice, but as discussed above, the equations of state are not analytic for any choice. (Indeed, this idea first spawned when the author was thinking about systematically rewriting equations in $\rho,h$.) We try three options: 1) fully random, 2) $p,T$ prefered, 3) $\rho,h$ prefered. This is implemented by added a bias to the initialization of the encoder coefficients,
