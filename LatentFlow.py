...
class LatentFlow(LatentSim):
    ...
    def make_line_mesh(self,Ny,L):
        ...
        self.X = np.c_[np.zeros((Ny,)),np.linspace(0,Ny,L)]
    ...
    def build_system(self,q,q0):
        ...
        vol_R0, = cf.Assemble(husk_identity.kernel_idty_R,
                                ...
                                {'R':(self.dm_q,),},
                                ndof=self.X.shape[1]*2)
        # Assemble the mass component at t+Dt:
        K_vol_arr,R_vol_arr = ls._sess.run([self.K_lhs,self.lhs],
        ...
        vol_R,vol_K = cf.Assemble(husk_identity.kernel_idty_RK,
                                    ...
                                    ndof=self.X.shape[1]*2)
        # Assemble the fluxes:
        q_face = np.array([ q[self.dm_q.Get_List(e[0:2])] for e in H_face ])
        X_face = np.array([ self.X.ravel()[self.dm_q.Get_List(e[0:2])] for e in H_face ])
        qA = q_face[:,0:2]
        qB = q_face[:,2:4]
        XA = X_face[:,0:2]
        XB = X_face[:,2:4]
        oF,oKF = ls._sess.run([self.o_F,self.o_KF],feed_dict={
            self.i_q:qA,self.i_q2:qB,
            self.i_XA:XA,self.i_XB:XB})
        flux_R,flux_K=cf.Assemble(husk_identity.kernel_idty_2_RK,
                                    self.H_face,
                                    {'iR':(oF.ravel(),self.dm_face),
                                     'iK':(oKF.ravel(),self.dm_face16)},
                                    {'R':(self.dm_q,),'K':(self.dm_q,)},
                                    ndof=self.X.shape[1]*2)
        #print(flux_R)
